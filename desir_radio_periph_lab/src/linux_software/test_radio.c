...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float clock_period = 1.0 / 125e6; // 125 MHz clock
    float time_spent = clocks_elapsed * clock_period;
    unsigned int bytes_transferred = 2048 * sizeof(unsigned int);
    float throughput = (bytes_transferred / time_spent) / 1e6;; 
...
int main()
{
    ...
    *(my_periph+RADIO_TUNER_CONTROL_REG_OFFSET) = 1; // reset in order to hear tune
